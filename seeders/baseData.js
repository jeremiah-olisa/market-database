...
export async function seedAreas(client) {
  ...
  try {
    ...
    for (const [name, details] of Object.entries(ABUJA_DISTRICTS)) {
      ...
      // Create a simple polygon around the center point for each district
      // This represents a rough district boundary (approximately 2km radius)
      const radius = 0.02; // Roughly 2km in degrees
      const geometry = `SRID=4326;MULTIPOLYGON(((
        ${coords.longitude - radius} ${coords.latitude - radius},
        ${coords.longitude + radius} ${coords.latitude - radius},
        ${coords.longitude + radius} ${coords.latitude + radius},
        ${coords.longitude - radius} ${coords.latitude + radius},
        ${coords.longitude - radius} ${coords.latitude - radius}
...
export async function seedEstates(client) {
  ...
  try {
    ...
    for (const area of areas) {
      const numEstates = randomInt(2, 4); // 2-4 estates per area
...
export async function seedEstateUnits(client) {
  ...
  try {
    ...
    await client.query('COMMIT');
    console.log('✅ Estate units seeded successfully');
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('❌ Error seeding estate units:', error);
    throw error;
  }
}
...
export async function seedPriceTrends(client) {
  ...
  try {
    ...
    await client.query('COMMIT');
    console.log('✅ Price trends seeded successfully');
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('❌ Error seeding price trends:', error);
    throw error;
  }
}
